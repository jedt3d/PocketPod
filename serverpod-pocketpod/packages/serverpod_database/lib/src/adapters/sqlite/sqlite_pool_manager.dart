...
class SqlitePoolManager implements DatabasePoolManager {
  ...
  String get _resolvedFilePath {
    ...
    return path.join((_serverDirectory ?? Directory.current).path, filePath);
  }
  ...
  Future<void> _bootstrap() async {
    ...
    final sqliteOptions = _sqliteOptionsFromEnvironment(config);
...
SqliteOptions _sqliteOptionsFromEnvironment(SqliteDatabaseConfig config) {
  final profile = Platform.environment['SERVERPOD_SQLITE_PROFILE'];
  final maxReaders =
      config.maxConnectionCount ?? SqliteOptions.defaultMaxReaders;

  if (profile == 'untuned') {
    return const SqliteOptions(
